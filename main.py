...
class BlockChain:
    ...
    def construct_block(self, proof_no, prev_hash):
        block = Block(
            index=len(self.chain), proof_no=proof_no, prev_hash=prev_hash, data=self.current_data,
        )
        self.current_data = []
...
blockchain.new_data(
    sender="0",  # it implies that this node has created a new block
    recipient="Imprexis Tech Solution",  # let's send Imprexis Tech Solution some coins!
    quantity=5,  # creating a new block (or identifying the proof number) is awarded with 1
)

last_hash = last_block.calculate_hash
block = blockchain.construct_block(proof_no, last_hash)

print("***Mining Ra Coin has been successful***")
print(blockchain.chain)
